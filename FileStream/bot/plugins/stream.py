...
async def _copy_and_gen_link(message: Message):
    """
    Copy file to FLOG_CHANNEL → get message_id → gen HMAC link instantly.
    copy() is faster than forward() — no "Forwarded from" header overhead.
    HMAC is pure CPU — zero network wait after copy returns.
    """
...
async def private_receive_handler(bot: Client, message: Message):
    ...
    # user_exist DB write runs in background — doesn't block link generation
    asyncio.create_task(is_user_exist(bot, message))

    try:
        stream_link, file_name, file_size, mime_type = await _copy_and_gen_link(message)
        ...
        await bot.send_message(
            ...
            text=(
                f"Gᴏᴛ FʟᴏᴏᴅWᴀɪᴛ ᴏғ {e.value}s ғʀᴏᴍ "
...
async def channel_receive_handler(bot: Client, message: Message):
    if await is_channel_banned(bot, message):
        return
    asyncio.create_task(is_channel_exist(bot, message))

    try:
        stream_link, _, _, _ = await _copy_and_gen_link(message)
        ...
        await bot.send_message(
            chat_id=Telegram.ULOG_CHANNEL,
            text=f"ɢᴏᴛ ғʟᴏᴏᴅᴡᴀɪᴛ ᴏғ {w.x}s ғʀᴏᴍ {message.chat.title}\n\n**ᴄʜᴀɴɴᴇʟ ɪᴅ :** `{message.chat.id}`",
            disable_web_page_preview=True,
        )
